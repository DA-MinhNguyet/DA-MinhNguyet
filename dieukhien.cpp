...
void dieukhien::docMH() {
	...
	while (!filein.eof()) {
		...
		m->setInfo(MAHANG, TENHANG, NHOMMH, GIA);
		listMH.push_back(m);
	}
	filein.close();
}
...
void tieude() {
	cout << left << "\n\t\t" << setw(10) << "Ma MH" << setw(20) << "Ten MH"<< setw(20)<<"Nhom san pham";
...
void dieukhien::lapHD() {
	...
	cout << left << "\n\t\t" << "Tong tien: " << (size_t)tongtien << endl;
	cout << "\n\t\tMa khach hang: " << idkh << endl;
	cout << "\n\t\tHo ten: " << hoten << endl;
}
...
void dieukhien::menu() {
	...
	while (true) {
		...
		case 2: {
			//this->docMH();
			cout << "\n\t\t" << left << setw(10) << "Ma hang" << setw(20) << "Ten hang"<< setw(20) << "Nhom san pham" << setw(20) << "Gia" << endl;
